...
### Agent Runtime Operations Umbrella

This section absorbs several narrow Hermes-operations skills into the main Hermes Agent class-level skill.

#### Agent state calibration

Use for local agent state files such as `AGENTS.md`, `MEMORY.md`, project dashboards, compact working-memory summaries, and active/stale loop inventories. Treat these as operational state, not generic notes: keep them concise, declarative, current, and scoped to the workspace/profile they affect. When editing memories, write durable facts rather than session progress.

#### Skill authoring

For reusable workflows, author complete `SKILL.md` packages with frontmatter, a class-level trigger description, actionable steps, pitfalls, and verification. Prefer patching an existing umbrella skill over creating a one-session narrow sibling. Put reusable details in `references/`, starter files in `templates/`, and runnable helpers in `scripts/`.

#### macOS background computer use

Hermes can drive macOS apps in the background with `computer_use` without stealing the user's cursor or Space. Capture with `mode='som'`, click by numbered element, verify after state changes, and never interact with permission, password, payment, or secret-entry UI unless the user explicitly directs safe scope.

#### Kanban orchestrator and worker flows

Kanban orchestration is a routing workflow: the orchestrator decomposes and dispatches instead of doing the work itself; workers orient, execute, heartbeat, block/complete with evidence, and keep workspace state clean. Do not mix orchestrator and worker responsibilities in the same run.
